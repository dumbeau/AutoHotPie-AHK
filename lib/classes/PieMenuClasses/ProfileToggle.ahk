class ProfileToggle{
__New(ProfileToggleSettings, _appProfile:=""){
	ObjMerge(this, ProfileToggleSettings)

	this.__Parent := &_appProfile
...
enablePieToggle(){
	this.toggleEnableState(this.appProfiles[1])
	...
	Else
		{
		this.toggleEnableState(this.appProfile)
		Hotkey, IfWinActive, % "ahk_group " . profile.groupName
		
		}
	for k, pieKey in profile.appProfile.pieKeys
		{
